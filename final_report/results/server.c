...
int debug_mode = 1; // デバッグログの切り替えフラグ
...
void cmd_check(int fd) {
    char buf[MAX];
    int len = sprintf(buf, "%d profile(s)\n", profile_data_nitems);
    if (debug_mode) printf("%s", buf);
    write(fd, buf, len);
}
...
void cmd_read(int fd, char *filename) {
    ...
    fclose(file);
    int len = sprintf(buf, "Data read from %s successfully (%d items).\n", filename, count);
    if (debug_mode) printf("%s", buf);
    write(fd, buf, len);
}
...
void cmd_write(int fd, char *filename) {
    ...
    fclose(file);
    int len = sprintf(buf, "Data written to %s successfully.\n", filename);
    if (debug_mode) printf("%s", buf);
    write(fd, buf, len);
}
...
void cmd_find(int fd, char *word) {
    ...
    int len = sprintf(buf, "%d item(s) found.\n", count);
    if (debug_mode) printf("%s", buf);
    write(fd, buf, len);
}
...
void cmd_sort(int fd, int column) {
    ...
    int len = sprintf(buf, "Sorted by column %d.\n", column);
    if (debug_mode) printf("%s", buf);
    write(fd, buf, len);
}
...
void cmd_delete(int fd, int id) {
    ...
    int len = sprintf(buf, "%d profile(s) deleted.\n", count);
    if (debug_mode) printf("%s", buf);
    write(fd, buf, len);
}
    ...
    if (line[0] == '%') {
        ...
        if (profile_data_nitems < MAX_PROFILES) {
            ...
            if (res != NULL) {
                profile_data_store[profile_data_nitems++] = new_p;
                char buf[MAX];
                int len = sprintf(buf, "Profile added successfully.\n");
                if (debug_mode) printf("%s", buf);
                write(fd, buf, len);
            } else {
                char buf[MAX];
...
int main() {
    ...
    listen(s, 5);
    printf("Server started. Waiting for connection on port 61001...\n");
    ...
    while (1) {
        ...
        while (1) {
            ...
            while (total_received < MAX - 1) {
                ...
                for (int i = 0; i < n; i++) {
                    if (recv_buf[total_received + i] == '\0') {
                        found_null = 1;
                        break;
                    }
                }
