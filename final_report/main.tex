...
アクセス時には，\texttt{address = base + (i * cols + j) * 4} という計算式をアセンブリ命令として展開し，
...
\verb|FizzBuzz| プログラムの実行結果（30,777命令）を見ると，その相当数が \verb|nop| であると推測される．
...
% Fizzbuzzがおかしいのでみてみる必要あり。
% 実行結果のところから書く。
% レポートの要件から必要なものを全て書いているかチェックする。
